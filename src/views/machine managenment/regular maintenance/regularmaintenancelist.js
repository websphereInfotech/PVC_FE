  ...
  return (
    <Card style={{ width: '100%', padding: '20px' }}>
        ...
        <Table style={{ border: '1px solid lightgrey' }}>
          ...
          <TableBody>
            {machineData?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
              <TableRow
                key={row.id}
                sx={{ backgroundColor: index % 2 === 0 ? 'white' : 'rgba(66, 84, 102, 0.1)' }}
      ...
      <Dialog open={openDetailsDialog} onClose={handleCloseDetailsDialog}>
        <DialogTitle>Regular Maintenance Details</DialogTitle>
        <DialogContent>
          <TextField
            label="Cost"
            ...
            fullWidth
            margin="dense"
            onChange={(e) => setSelectedRow({ ...selectedRow, cost: e.target.value })}
          />
          <TextField
            label="Description"
          ...
          <TextField
            label="Date"
            type="date"
            value={selectedRow.date ? new Date(selectedRow.date).toISOString().split('T')[0] : ''}
            fullWidth
            margin="dense"
            InputLabelProps={{
              shrink: true
        ...
        <DialogActions>
          <Button variant="contained" onClick={handleCloseDetailsDialog} color="secondary">
            Close
          </Button>
        </DialogActions>
      </Dialog>
